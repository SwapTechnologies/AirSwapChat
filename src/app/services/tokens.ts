...
//     'name': 'VeChain',
//     'symbol': 'VEN',
//     'url': 'https://tokensale.vechain.com/en/',
//     'decimals': '18',
//     'labels': [
//       'enterprise'
//     ],
...
export const getTokenByName = (name) => {
    ...
    while (l <= h) {
        ...
        const mName = EthereumTokensSN[m].name.toLocaleLowerCase();
        const c = nameLow < mName ? -1 : nameLow === mName ? 0 : 1;
...
export let EthereumTokensSA = EthereumTokens.slice(0);
...
export const getTokenByAddress = (address) => {
    ...
    while (l <= h) {
        const m = Math.floor((l + h) / 2);
        const mAddr = EthereumTokensSA[m].address.toLocaleLowerCase();
        const c = addressLow < mAddr ? -1 : addressLow === mAddr ? 0 : 1;
        if (c === 0) {
...
const addTokenByAddressFromEthplorer = (address) => {
...
const addAndUpdateTokenLists = (newToken) => {
